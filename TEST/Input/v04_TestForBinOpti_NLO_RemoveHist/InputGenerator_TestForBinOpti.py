...
for massBinEdgePair in list_massBinEdgePair:
    ...
    for i in range( 0, len(list_diRapBinEdge)-1 ):
        ...
        # -- mandatory options
        i_rapBin = i+1
        generator.tag = "TestForBinOpti_TEST_M%.0lfto%.0lf_diRap%.1lfto%.1lf" % (minM, maxM, minY, maxY)
        generator.tag = generator.tag.replace(".", "p")
        ...
        generator.customHistPath = "./histTemplate_M%dto%d.txt" % (minM, maxM)
...
scriptGenerator.tag = "TestForBinOpti_TEST"
